...
<section class="bg-purple-custom dark:bg-gray-950 dark:text-white w-64">
        <div class="flex flex-col items-center h-full">
          <div class="flex items-center justify-center w-16 h-16 my-5 rounded-full">
            <img src="./img/amir.webp" alt="logo" class="w-full" />
...
`;class x extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.appendChild(u.content.cloneNode(!0))}}let o=document;window.customElements.define("side-bar",y);window.customElements.define("header-bar",x);let p;const d=o.querySelector(".the-product-holder"),g=o.querySelector("#addNew"),h=o.querySelector("#modal2"),f=o.querySelector("#modal"),w=o.querySelector("#save-edit"),v=o.querySelector("#search-input"),b=o.querySelector("#edit-product-name"),E=o.querySelector("#edit-category"),L=o.querySelector("#edit-price"),S=o.querySelector("#product-name"),M=o.querySelector("#price"),j=o.querySelector("#category"),N=o.querySelector("#save-btn");window.addEventListener("load",()=>{console.log(),l()});window.deleteProduct=q;window.openEditModal=C;function l(){fetch("https://js-cms-json-api.liara.run/product").then(e=>e.json()).then(e=>{d.innerHTML="",e.forEach(t=>{d.insertAdjacentHTML("beforeend",`
                    ...
                    `)})})}function m(){const e=()=>Math.floor(100+Math.random()*900),t=()=>String.fromCharCode(65+Math.floor(Math.random()*26));return`#${e()}-${e()}${t()}${t()}${t()}`}console.log(m());function P(){let e={productName:S.value,price:M.value,category:j.value};fetch("https://js-cms-json-api.liara.run/product",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(e)}).then(t=>{l(),$()})}function $(){h.classList.add("hidden")}function q(e){fetch(`https://js-cms-json-api.liara.run/product/${e}`,{method:"DELETE"}).then(t=>{console.log(t),l()})}function k(){let e={productName:b.value,price:L.value,category:E.value};fetch(`https://js-cms-json-api.liara.run/product/${p}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)}).then(t=>{l()}),I()}function C(e){p=e,f.classList.remove("hidden")}function I(){f.classList.add("hidden")}g.addEventListener("click",()=>{h.classList.remove("hidden")});w.addEventListener("click",()=>{k()});N.addEventListener("click",e=>{e.preventDefault(),P()});v.addEventListener("keyup",e=>{if(e.key==="Enter"){let t=e.target.value.toLowerCase();document.querySelectorAll(".the-product-holder tr").forEach(n=>{let r=n.textContent.toLowerCase();n.style.display=r.includes(t)?"":"none"})}});
